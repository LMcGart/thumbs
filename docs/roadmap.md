...
- [x] **10. Onboarding.** *(Final form 2026-08-29 after on-device testing: detection cut — the sure-tier stack (dedupe, dominant-nearest, meal filter, OCR receipt matching) yielded ~zero certain cards at city POI density, and anything less than certain erodes first-run trust. Onboarding is: welcome → handle/name → find friends → home. Detection returns when the app's own visit data provides a which-place-do-humans-pick prior — see docs/later.md.)*
  *Done when:* the flow completes in well under 90 s, can't re-trigger accidentally, and lands on home signed in with a handle.
