...
- [ ] **10. Calibration onboarding.** *(Simplified 2026-08-29, revised after on-device test: the Vision meal check is back as a card filter — without it, any photo near a food POI became a card; declined photo access goes straight to find-friends, the well-known-places grid is dropped; rating a card reuses the standard rating flow preset with the detected visit date.)* After sign-in: explain, ask for photo access, then detect the user's **last 10 distinct places**: scan backwards from today (180-day max lookback — older visits can't be rated from memory), cluster, walk most-recent-first, and surface candidates until 10 distinct places are collected — single-candidate clusters as "rate it", ambiguous ones as "X or Y?" (band-mates appear from the second rating onward), skip allowed per item. Food checks run lazily during the walk so most hit local full-res photos, and "couldn't check in time" means skip, not block. Fallback if photo access declined: grid of ~20 well-known NYC places, "which have you been to?", rate the selected ones. Then a find-friends screen (by handle; contacts later). Land on home.
