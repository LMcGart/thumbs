...
- [x] **2. Clustering.** `Core/Detection`: a pure function from `[PhotoSample]` (id, date, lat, lon) to `[Cluster]` (photos, centroid, start, end). Thresholds as parameters (default 2 h, 50 m). Ignores samples with no location.
...
- [ ] **10. Calibration onboarding.** *(Simplified 2026-08-29, revised after on-device test: the Vision meal check is back as a card filter, and cards are sure-tier only (single candidate, ≥2 photos, meal signal) — no disambiguation pickers after on-device testing showed ambiguous cards erode trust; declined photo access goes straight to find-friends, the well-known-places grid is dropped; rating a card reuses the standard rating flow preset with the detected visit date.)* After sign-in: explain, ask for photo access, then detect the user's **last 10 distinct places**: scan backwards from today (180-day max lookback — older visits can't be rated from memory), cluster, walk most-recent-first, and surface candidates until 10 distinct places are collected — single-candidate clusters as "rate it", ambiguous ones as "X or Y?" (band-mates appear from the second rating onward), skip allowed per item. Food checks run lazily during the walk so most hit local full-res photos, and "couldn't check in time" means skip, not block. Fallback if photo access declined: grid of ~20 well-known NYC places, "which have you been to?", rate the selected ones. Then a find-friends screen (by handle; contacts later). Land on home.
  *Done when:* both paths complete in under 90 s and produce rated places that appear on the profile; a heavy eater gets ~2 recent weeks and a light eater reaches back further, both capped at 10 cards; onboarding can't be re-triggered accidentally.

## Phase 3 — Reservation drops

- [x] ~~**11. Reservation drops.**~~ **Cut from v1** (2026-08-29, product decision): not necessary. Schema tables (`drop_rules`, `reminders`) stay dormant. See docs/later.md. `drop_rules` loaded from `docs/drop-rules.csv` (user-verified; columns: place, platform, rule_type, window_days, time_local, weekday, phone, url, confidence, verified_on). Rule engine in `Core/Places`: rule + desired date → drop moment(s), all rule types in CLAUDE.md Detection/Places specs, Eastern time, "already released" case. Restaurant-page widget: rule sentence, confidence, verified date, "this is wrong" (writes a report row). CTA: multi-date picker → one calendar event (write-only permission) + local notification per drop moment, collapsed when moments coincide → link or phone.
